...
public class MlkitPlugin implements MethodCallHandler {
    ...
    public static void registerWith(Registrar registrar) {
        final MethodChannel channel = new MethodChannel(registrar.messenger(), "ml_kit_flutter/arenum");
    ...
    private static int toDim(ArrayList<Integer> list) {
        int l = list.size();
        int dim = 1;
        Iterator<Integer> iter = list.iterator();
        for (int i = 0; i < l; i++)
    ...
    public void onMethodCall(MethodCall call, final Result result) {
        ...
        if (call.method.endsWith("#detectFromPath")) {
            /// Step 9
            /// Конвертируем File -> Bitmap -> FirebaseVisionImage
            /// Коротко что нужно сделать:
            ///     1) Взять файл
            ///     2) Засунуть в BitmapFactory.decodeFile
            ///     3) Перевернуть изображение если это нужно (данный код будет реализован ниже)
            ///     4) Из Bitmap конвертировать в FirebaseVisionImage при помощи метода fromBitmap
            ///
            /// BitmapRotation:
        ...
        if (call.method.startsWith("FirebaseVisionTextDetector#detectFrom")) {
            /// Step 10
            /// Запускам ML на вашем устройстве!
            /// Для разпознования теста в Mlkit существует FirebaseVisionTextRecognizer.
            /// Мы будем использовать метод getOnDeviceTextRecognizer, который возвращает вам FirebaseVisionText
            /// 
            /// Для того чтобы запустить вам нужно вызвать processImage метод и передать FirebaseVisionImage и
            /// обработать addOnSuccessListener и addOnFailureListener которые будут возвращать вам данные во Flutter.
            /// 
            /// Вернуть данные во Flutter чать можно при помощи вызова result
            /// Есть 3 состояния:
