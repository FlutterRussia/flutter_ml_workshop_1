...
class _CustomModelWidgetState extends State<CustomModelWidget> {
  ...
  Widget build(BuildContext context) {
    _models.forEach((model) {
      manager.registerRemoteModelSource(FirebaseRemoteModelSource(
          ...
          initialDownloadConditions: FirebaseModelDownloadConditions(requireWifi: true),
          updatesDownloadConditions: FirebaseModelDownloadConditions(requireWifi: true)));
    });
    _localModels.forEach((model) {
      manager.registerLocalModelSource(
          FirebaseLocalModelSource(modelName: model, assetFilePath: "assets/" + model + ".tflite"));
    });
          ...
          onPressed: () async {
            try {
              ...
              try {
                ...
                if (options.inputOptions[0].dataType == FirebaseModelDataType.BYTE) {
                  var bytes = await imageToByteListInt(_file, dim);
                  results = await interpreter.run(
                      localModelName: _localModels[_currentModel], inputOutputOptions: options, inputBytes: bytes);
                  factor = 2.55;
                } else {
                ...
                if (_currentModel == 2) {
                  for (var i = 0; i < results[0][0].length; i++) {
                    currentLabels.add(new ObjectDetectionLabel.box(
                        Box(results[0][0][i][0], results[0][0][i][1], results[0][0][i][2], results[0][0][i][3])));
                  }
                  for (var i = 0; i < results[1][0].length; i++) {
                    currentLabels[i].label = labels[_models[_currentModel]][results[1][0][i].round() + 1];
                  }
                  ...
                  for (var i = 0; i < results[0][0].length; i++) {
                    if (results[0][0][i] > 0) {
                      currentLabels
                          .add(new ObjectDetectionLabel(labels[_models[_currentModel]][i], results[0][0][i] / factor));
                    }
                  }
                }
  ...
  Future<Size> _getImageSize(Image image) {
    ...
    image.image.resolve(ImageConfiguration()).addListener(ImageStreamListener((ImageInfo info, _) {
      ...
      child: Container(
        child: ListView.builder(
            ...
            itemBuilder: (context, i) {
              return _buildRow(texts[i].label, texts[i].confidence, texts[i].color);
            }),
      ),
    );
  }
...
class _TextDetectPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    for (var text in _texts) {
      ...
      TextSpan span = new TextSpan(text: text.label, style: TextStyle(color: color));
