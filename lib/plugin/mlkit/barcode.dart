...
class FirebaseVisionBarcodeDetector {
  ...
  static FirebaseVisionBarcodeDetector instance = new FirebaseVisionBarcodeDetector._();
  ...
  Future<List<VisionBarcode>> detectFromBinary(Uint8List binary) async {
    try {
      List<dynamic> barcodes =
          await _channel.invokeMethod("FirebaseVisionBarcodeDetector#detectFromBinary", {'binary': binary});
      ...
      print("Error on FirebaseVisionBarcodeDetector#detectFromBinary : ${e.toString()}");
    }
    return null;
  }
  ...
  Future<List<VisionBarcode>> detectFromPath(String filepath) async {
    try {
      List<dynamic> barcodes =
          await _channel.invokeMethod("FirebaseVisionBarcodeDetector#detectFromPath", {'filepath': filepath});
      List<VisionBarcode> ret = [];
      barcodes?.forEach((dynamic item) {
        final VisionBarcode barcode = new VisionBarcode._(item);
        ret.add(barcode);
      ...
      print("Error on FirebaseVisionBarcodeDetector#detectFromPath : ${e.toString()}");
    }
    return null;
  }
}
...
class VisionBarcode {
  ...
  VisionBarcode._(this._data)
      : rect = Rect.fromLTRB(_data['rect_left'], _data['rect_top'], _data['rect_right'], _data['rect_bottom']),
        ...
        valueType = VisionBarcodeValueType.values.elementAt(_data['value_type']),
        email = _data['email'] == null ? null : VisionBarcodeEmail._(_data['email']),
        phone = _data['phone'] == null ? null : VisionBarcodePhone._(_data['phone']),
        sms = _data['sms'] == null ? null : VisionBarcodeSMS._(_data['sms']),
        url = _data['url'] == null ? null : VisionBarcodeURLBookmark._(_data['url']),
        wifi = _data['wifi'] == null ? null : VisionBarcodeWiFi._(_data['wifi']),
        geoPoint = _data['geo_point'] == null ? null : VisionBarcodeGeoPoint._(_data['geo_point']),
        contactInfo = _data['contact_info'] == null ? null : VisionBarcodeContactInfo._(_data['contact_info']),
        calendarEvent = _data['calendar_event'] == null ? null : VisionBarcodeCalendarEvent._(_data['calendar_event']),
        driverLicense = _data['driver_license'] == null ? null : VisionBarcodeDriverLicense._(_data['driver_license']);
}
...
class VisionBarcodeWiFi {
  VisionBarcodeWiFi._(Map<dynamic, dynamic> data)
      : ssid = data['ssid'],
        ...
        encryptionType = VisionBarcodeWiFiEncryptionType.values.elementAt(data['encryption_type']);
...
class VisionBarcodeContactInfo {
  VisionBarcodeContactInfo._(Map<dynamic, dynamic> data)
      : addresses = data['addresses'] == null
            ? null
            : data['addresses'].map<VisionBarcodeAddress>((dynamic item) => VisionBarcodeAddress._(item)).toList(),
        emails = data['emails'] == null
            ? null
            : data['emails'].map<VisionBarcodeEmail>((dynamic item) => VisionBarcodeEmail._(item)).toList(),
        name = data['name'] == null ? null : VisionBarcodePersonName._(data['name']),
        phones = data['phones'] == null
            ? null
            : data['phones'].map<VisionBarcodePhone>((dynamic item) => VisionBarcodePhone._(item)).toList(),
        urls = data['urls'] == null ? null : data['urls'].map<String>((dynamic item) => item).toList(),
...
class VisionBarcodeAddress {
  VisionBarcodeAddress._(Map<dynamic, dynamic> data)
      : addressLines = data['address_lines'].map<String>((dynamic item) => item).toList(),
